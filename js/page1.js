(function () {
    ...
    function sparkleLoopAddOne(phrases, atStart)
    {
    ...
    function sparkleLoop($scope) {
        ...
        if (!sparkle_loop_cycle) {
            sparkleLoopAddOne($scope.phrases, true);
        }
    ...
    function initSubPageSparkles($scope, subPage)
    {
        resetSparkles($scope);

        for(var i = 0; i < 10; i++)
        {
            sparkleLoopAddOne($scope.phrases, false);
        }
    }

    function setSparkleLoopRunning($scope, $timeout, $rootScope) {
        initSubPageSparkles($scope.mySubPage);
